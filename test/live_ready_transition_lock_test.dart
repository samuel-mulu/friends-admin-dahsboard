    ...
    test('9. TTL expiry clears lock', () {
      ...
      expect(
        shouldClearReadyTransitionLock(
          ...
          pinnedGame: _readyGame(sessionId: 'session-a'),
          now: now.add(kReadyTransitionLockTimeout),
        ),
        isTrue,
      );
    });

    test('10. expired lock is not active for resolver', () {
      final lock = _lock(
        game: _readyGame(sessionId: 'session-a'),
        reason: ReadyTransitionReason.preparingToPlay,
      );
      expect(lock.isActiveAt(now), isTrue);
      expect(lock.isActiveAt(now.add(kReadyTransitionLockTimeout)), isFalse);
      ...
      expect(primary?.sessionId, 'session-b');
    });
